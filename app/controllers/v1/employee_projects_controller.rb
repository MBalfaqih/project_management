class V1::EmployeeProjectsController < ApplicationController
    ...
    def update
        if updated_ids = params[:ids]
            ...
            render_success message: I18n.t("employee_updated_successfully"), data: V1::EmployeeDetailsSerializer.new(@employee)  
        else 
            EmployeesProject.remove_unchecked_projects(@employee.project_ids , @employee)
            
            render_success message: I18n.t("employee_projects_deleted"), data: V1::EmployeeDetailsSerializer.new(@employee)
        end
    end
